...
/********************************************************************************************* READ ME
 *  MIT WORKSHOP USERS: to use this version of TangoARScreen.cs:
 *  
 *      open Assets/TangoPrefabs/Scripts/TangoARScreen.cs and delete everything after line 24
 *      copy everything here inside the namespace (NOT including the namespace itself)
 *      paste into line 24 of TangoARScreen.cs
 *      
 *  short version: Tango dependencies expect TangoARScreen to be found in the global namespace
 *********************************************************************************************/

namespace TangoWorkshop_Part_2_Completed
{
    ...
    {
        /// <summary>
        /// TANGO WOKRSHOP - If set, this RenderTexture will receive the RGB video
        /// instead of the camera rendering it to the screen.
        /// </summary>
        public RenderTexture videoRGB;
        ...
        public void Start()
        {
            ...
            if (tangoApplication != null)
            {
                ...
                // TANGO WORKSHOP - Copy to videoRGB and clear if videoRGB is set.
                if (videoRGB)
                {
                    CommandBuffer extract = new CommandBuffer();
                    extract.Blit((Texture)null, videoRGB);
                    extract.ClearRenderTarget(true, true, Color.black);
                    m_camera.AddCommandBuffer(CameraEvent.BeforeForwardOpaque, extract);
                }
